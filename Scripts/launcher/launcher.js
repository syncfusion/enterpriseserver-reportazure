...
function launchIE(url) {
    ...
    if (navigator.msLaunchUri) {
        navigator.msLaunchUri(url, function msLaunchUriSuccess() {
            isSupported = true;
            result();
...
$(document).on("click", "#launcher", function (e) {
    if (i_am_ie9) {
        e.preventDefault();
        $("#messageBox_wrapper, .e-dialog-scroller, #messageBox").addClass("ie-msgbox-height");      
        messageBox("su-info", "[[[Information]]]", "[[[We are unable to launch the Report Designer from IE 9 browser. Please refer to this <a href='https://help.syncfusion.com/report-platform/reportdesigner/installation'>link</a> on how to open a Report in Report Designer]]]", "success", function () {
            onCloseMessageBox();
        });
    }
    else {
